...
var content = fs.readFileSync('data/5.txt');
...
$("td[style='border-bottom:1px solid #e3e3e3; width:260px']").each(function(i, elem) {
    ...
    if ($(elem).html().split('<b>')[1].trim().split('<br>')[0].trim().split(',')[0].split('-')[0].replace("&apos;","'").replace("\'","'").trim() == "WOMEN TOGETHER                      (:I)"){
        groupName.push("WOMEN TOGETHER (:I)");
    } else {
        groupName.push($(elem).html().split('<b>')[1].trim().split('<br>')[0].trim().split(',')[0].split('-')[0].replace("&apos;","'").trim().replace("&amp;","&"));
    }
...
, function() {
    fs.writeFileSync('data/zone3.json', JSON.stringify(meetingsData));
    // console.log('*** *** *** *** ***');
