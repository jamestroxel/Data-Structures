...
var content = fs.readFileSync('data/6.txt');
...
$("td[style='border-bottom:1px solid #e3e3e3; width:260px']").each(function(i, elem) {
    ...
    if ($(elem).html().split('<b>')[1].trim().split('<br>')[0].trim().split(',')[0].split('-')[0].replace("&apos;","'").replace("\'","'").trim() == "NINTH AVENUE           (:II)"){
        groupName.push("NINTH AVENUE (:II)");
    } else {
        groupName.push($(elem).html().split('<b>')[1].trim().split('<br>')[0].trim().split(',')[0].split('-')[0].replace("&apos;","'").trim().replace("&amp;","&"));
    }
...
, function() {
    fs.writeFileSync('data/zone2.json', JSON.stringify(meetingsData));
    // console.log('*** *** *** *** ***');
